...
Formal languages tend to have strict rules about syntax. For example, 3 + 3 = 6 is a syntactically correct mathematical statement, but 3 + = 3 $ 6 is not. *H<sub>2</sub>O* is a syntactically correct chemical formula, but <sub>2</sub>Z<sub>z</sub> is not.

Syntax rules come in two flavors, pertaining to **tokens** and structure. Tokens are the basic elements of the language, such as words, numbers, and chemical elements. One of the problems with 3 + = 3 $ 6 is that $ is not a legal token in mathematics (at least as far as I know). Similarly, <sub>2</sub>Z<sub>z</sub> is not legal because there is no element with the abbreviation Z<sub>z</sub>.
